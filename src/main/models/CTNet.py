...
if __name__ == "__main__":
    ...
    for in_channels, out_num, sfreq in [(22,4,250),(4,2,250),(16,2,250),(62,2,250)]:
        print(f"\n==== CTNet: in_channels={in_channels}, out_num={out_num}, sfreq={sfreq} ====")
        emb_size = 40
        nTime = 750
        flatten_eeg1 = nTime * emb_size
        m = CTNet(eeg1_number_channel=in_channels, number_class=out_num, flatten_eeg1=flatten_eeg1).to(device)
        summary(m, input_size=(1, 1, in_channels, nTime), device=device)
        inputs = torch.randn(1, 1, in_channels, nTime).to(device)
        flops, params = profile(m, inputs=(inputs,))
